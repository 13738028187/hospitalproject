How does it work?
------------------
state.js uses localStorage when available, and falls back on the userData behavior in IE6 and IE7. No flash to slow down your page load. No cookies to fatten your network requests.

state.js depends on JSON for serialization to disk.
...
`store.enabled` flag
--------------------
If your product depends on state.js, you must check the `store.enabled` flag first:
...
Contributors & Forks
--------------------
Contributors: https://github.com/marcuswestin/state.js/graphs/contributors

Forks: https://github.com/marcuswestin/state.js/network/members


In node.js
----------
state.js works as expected in node.js, assuming that global.localStorage has been set:
...
*Private mode* Store.js may not work while browsing in private mode. This is as it should be. Check the `store.enabled` flag before relying on state.js.

*Saucelabs.com rocks* Extensive browser testing of state.js is possible thanks to Saucelabs.com. Check them out, they're awesome.

*Firefox 3.0 & 2.0:* Support for FF 2 & 3 was dropped in v1.3.6. If you require support for ancient versions of FF, use v1.3.5 of state.js.
...
-------------------
 ...
 - Safari 3: no synchronous api (has asynch sqlite api, but state.js is synch)
...
---------------------------
localStorage, when used without state.js, calls toString on all stored values. This means that you can't conveniently store and retrieve numbers, objects or arrays:
...
localStorage.tags = ['javascript', 'localStorage', 'state.js']
localStorage.tags.length === 32
localStorage.tags === state.js
```

What we want (and get with state.js) is
...
store.set('tags', ['javascript', 'localStorage', 'state.js'])
alert("We've got " + store.get('tags').length + " tags here")
```

The native serialization engine of javascript is JSON. Rather than leaving it up to you to serialize and deserialize your values, state.js uses JSON.stringify() and JSON.parse() on each call to store.set() and store.get(), respectively.
...
----------------------------------
No. I believe there is no way to provide sessionStorage semantics cross browser. However, it is trivial to expire values on read on top of state.js:
