...
function runStorageTests(store) {
	...
	// If plain local storage was used before state.js, we should attempt to JSON.parse them into javascript values.
	// Store values using vanilla localStorage, then read them out using state.js
	var promoteValues = {
		'int'         : 42,
		'bool'        : true,
		'float'       : 3.141592653,
